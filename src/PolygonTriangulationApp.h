...
static std::unique_ptr<PolygonTriangulation> triangulationPtr;
...
void updateVBO()
{
	if (pointCount > 0 && triangulationPtr)
	{
		glBindBuffer(GL_ARRAY_BUFFER, vbo);
		glBufferData(GL_ARRAY_BUFFER,
					 sizeof(Vector2f) * pointCount,
					 triangulationPtr->vertexData(),
					 GL_STATIC_DRAW);
	}
}
...
void extractIndices(const PolygonTriangulation& mesh)
{
...
void mouse_button_callback(GLFWwindow* /*window*/, int button, int action, int /*mods*/)
{
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
	{
		if (triangulationPtr)
		{
			triangulationPtr->advance();
			extractIndices(*triangulationPtr);
		}
	}
}
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (key == GLFW_KEY_ENTER && action == GLFW_PRESS)
	{
		if (triangulationPtr)
		{
			while (triangulationPtr->advance())
			{
				extractIndices(*triangulationPtr);
